...
export default function WaitlistForm({ onSuccess }: { onSuccess: (s: WaitlistSuccess) => void }){
  ...
  async function submit(e: React.FormEvent){
    e.preventDefault(); if(!emailValid){ setEmailError('Enter a valid email'); emailRef.current?.focus(); return }
    ...
    }catch(err){ setError(err instanceof Error? err.message : 'Something went wrong. Try again.') } finally{ setPending(false) }
  }
    ...
    <form onSubmit={submit} noValidate className="w-full">
      <div className="rounded-[18px] bg-white ring-1 ring-black/10 p-3 sm:p-4 shadow-[0_16px_48px_rgba(12,20,24,0.18)]">
        ...
        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
          <label className="block">
            <span className="block font-mono text-[10px] tracking-[0.14em] text-zinc-500 uppercase">Current setup <span className="normal-case tracking-normal font-normal text-zinc-400">— valgfritt</span></span>
            <div className="relative mt-1.5">
              <select value={current} onChange={e=>setCurrent(e.target.value)} className="w-full h-[40px] rounded-[10px] bg-zinc-50 ring-1 ring-zinc-200 px-3 pr-8 text-[13px] text-zinc-700 outline-none focus:bg-white focus:ring-2 focus:ring-[var(--color-harbour-ring)] appearance-none">
                <option value="" className="bg-white">Select — optional</option>
